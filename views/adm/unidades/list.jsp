...
<%
    List<Unidad> unidades = Unidad.getAll();
%>
...
<div class="admin-toolbar" style="margin-bottom: 20px;">
    <div class="toolbar-search">
        ...
        <input type="text" id="searchInput" placeholder="Buscar unidad..." oninput="filterTable()">
    </div>
    ...
    <div class="admin-card-body">
        <% if (unidades.isEmpty()) { %>
            <div class="empty-state">
                ...
                <div class="empty-state-desc">Crea tu primera unidad para la seccion de unidades</div>
                <button class="btn-admin btn-admin-primary" onclick="openModal()">
                    <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
                    Crear unidad
                </button>
        ...
        <% } else { %>
            ...
            <table class="admin-table" id="unidadesTable">
                <thead>
                    <tr>
                ...
                <tbody id="tableBody">
                    ...
                    <tr data-id="<%= u.id %>" data-active="<%= u.activo %>">
                        <td data-label="">
                        ...
                        <td data-label="Titulo">
                            <span class="table-title" style="font-size: 0.85rem;"><%= u.titulo %></span>
                        </td>
                        <td data-label="Descripcion" class="col-desc">
                            <span class="table-text-truncate" style="max-width: 280px; display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--default-color);"><%= u.descripcion != null ? u.descripcion : "" %></span>
                        </td>
                        <td data-label="Enlace" class="col-enlace">
                            <% if (u.enlaceUrl != null && !u.enlaceUrl.isEmpty()) { %>
                                <a href="<%= u.enlaceUrl %>" target="_blank" class="table-link" style="font-size: 0.78rem; white-space: nowrap;">
                                    <%= u.enlaceTexto != null && !u.enlaceTexto.isEmpty() ? u.enlaceTexto : "Ver" %>
                                </a>
                            <% } else { %>
                        ...
                        <td data-label="Estado">
                            <span class="badge-status <%= u.activo ? "badge-active" : "badge-inactive" %>" style="font-size: 0.65rem; padding: 2px 8px;">
                        ...
                        <td data-label="Acciones">
                            <div class="action-btn-group" style="gap: 4px;">
                                <button class="action-btn" title="Editar" style="width: 28px; height: 28px;"
                                    data-id="<%= u.id %>"
                                    data-titulo="<%= u.titulo != null ? u.titulo : "" %>"
                                    data-descripcion="<%= u.descripcion != null ? u.descripcion : "" %>"
                                    data-enlace-url="<%= u.enlaceUrl != null ? u.enlaceUrl : "" %>"
                                    data-enlace-texto="<%= u.enlaceTexto != null ? u.enlaceTexto : "" %>"
                                    data-enlace-nueva-pestana="<%= u.enlaceNuevaPestana %>"
                                    data-orden="<%= u.orden %>"
                                ...
                                <button class="action-btn danger" title="Eliminar" style="width: 28px; height: 28px;" onclick="confirmDelete('${pageContext.request.contextPath}/adm/unidades/delete?id=<%= u.id %>')">
...
<!-- Modal Form -->
<div class="modal fade modal-admin" id="unidadModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form method="POST" action="${pageContext.request.contextPath}/adm/unidades/save" class="form-admin" id="unidadForm">
                    ...
                    <div class="row g-3">
                        <div class="col-md-8">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="formTitulo" name="titulo" required placeholder="Titulo de la unidad">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                    ...
                    <div class="row g-3">
                        <div class="col-md-8">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" id="formDescripcion" name="descripcion" rows="3" placeholder="Descripcion breve de la unidad"></textarea>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
...
function openModal(btn) {
    var modal = document.getElementById('unidadModal');
...
function saveOrder() {
    ...
    fetch('${pageContext.request.contextPath}/adm/unidades/reorder', {
        method: 'POST',
